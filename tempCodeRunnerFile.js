...
app.post("/register", async (request,response)=>{
    try{
        ...
        const ashedPassword = await bcrypt.hash(password,salt);
        ...
        response.status(201).json({
            message:"The use has registered Succesfully!!!"
        });
    }
    catch(error){
...
app.put("/hotels/:id", isAuthenticated, (request,response) => {
    try{
        ...
        if(name){
            const nameExists = hotel.find((h)=>h.name.toLowerCase()===name.toLowerCase() && h.id !=request.params.id);
                if (nameExists){
                    return response.status(400).json({message:"Another hotel with this already exits."})
                }
            hotel.name=name;
        }
        ...
        response.status(500).json({ error: error.message }); // FIX: spelling
    }
});
